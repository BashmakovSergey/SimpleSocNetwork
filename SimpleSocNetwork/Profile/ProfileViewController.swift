...
final class ProfileViewController: UIViewController {
    ...
    public lazy var profileIDLabel: CustomLabel = {
        let label = CustomLabel(labelText: "ID", fontsize: 16, fontColor: .textMainBlack, alignment: .left)
        return label
    }()
    ...
    public lazy var profileTableView: UITableView = {
        ...
        tableView.backgroundColor = .backgroundMainGray
...
extension ProfileViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete".localized) { [weak self] _,_,_ in
            ...
            if let post = profile?.posts?.allObjects[indexPath.row] as? PostData {
